...
inThisBuild(Seq(
  ...
  organization := "org.codeape.timepool",
  scalacOptions ++= Seq(
    "-feature",
    "-deprecation",
    "-unchecked",
...
val commonSettings = Seq(
  moduleName := "timepool-" + moduleName.value,
)
...
lazy val root = project.in(file("."))
  ...
  .settings(
    ...
    Compile / mainClass := Some("org.codeape.timepool.backend.Launcher")
  )
...
lazy val frontend = project.in(file("frontend"))
  ...
  .settings(
    ...
    Compile / mainClass := Some("org.codeape.timepool.frontend.JSLauncher"),
    ...
    compileCss := Def.taskDyn {
      ...
      dir.mkdirs()
      (backend / Compile / runMain).toTask(s" org.codeape.timepool.backend.css.CssRenderer $path false")
    }.value,
